/*The purpouse of this autonomous mode is to go straight, as to clear as many blocks
 ...
 waiting 10 seconds before starting from the close blue postition*/
...
public class AutonomousBlueCloseStraightFloorGoalWait extends OpMode {
    ...
    public AutonomousBlueCloseStraightFloorGoalWait() {
    }
    ...
    public void loop() {
        ...
        if (currentTime > 11 && !Treadleft.isBusy() && !Treadright.isBusy()){
            ...
            Treadleft.setTargetPosition(currentLeftEncoderPosition - 1137);
            Treadright.setTargetPosition(currentRightEncoderPosition + 1252);
            Treadleft.setMode(DcMotorController.RunMode.RUN_TO_POSITION);
            Treadright.setMode(DcMotorController.RunMode.RUN_TO_POSITION);
