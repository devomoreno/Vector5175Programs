...
/*The purpouse of this autonomous mode is to go straight, as to clear as many blocks
 ...
 waiting 10 seconds starting from the FAR RED postition*/
...
public class AutonomousRedFarStraightFloorGoalWait extends OpMode {
    ...
    public AutonomousRedFarStraightFloorGoalWait() {
    }
    ...
    public void loop() {
        ...
        if (currentTime > 11 && !Treadleft.isBusy() && !Treadright.isBusy()){
            ...
            Treadleft.setTargetPosition(currentLeftEncoderPosition + 889);
            Treadright.setTargetPosition(currentRightEncoderPosition - 937);
            Treadleft.setMode(DcMotorController.RunMode.RUN_TO_POSITION);
            Treadright.setMode(DcMotorController.RunMode.RUN_TO_POSITION);
